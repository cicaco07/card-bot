...
def test_slash_command_snapshot() -> None:
    ...
    assert snapshot == [
        ...
        ("poker-start", "Tampilkan meja Remi Poker interaktif di channel ini.", [("mode", False), ("rounds", False), ("table_name", False)]),
        ("poker-hand", "Fallback: lihat kartu Remi Poker tanganmu secara private.", [("table_code", False)]),
        ("poker-status", "Fallback: refresh status meja Remi Poker.", [("table_code", False)]),
        ("rummy-start", "Tampilkan meja Rummy interaktif di channel ini.", [("mode", False), ("rounds", False), ("table_name", False)]),
        ("rummy-hand", "Fallback: lihat kartu Rummy tanganmu secara private.", [("table_code", False)]),
        ("rummy-status", "Fallback: refresh status meja Rummy.", [("table_code", False)]),
        ("tournament-list", "Lihat meja tournament persistent aktif di server ini.", [("mode", False)]),
        ("tournament-resume", "Buka kembali panel tournament dari checkpoint akhir ronde.", [("table_code", True)]),
        ("tournament-archive", "Arsipkan meja tournament persistent.", [("table_code", True)]),
    ]
