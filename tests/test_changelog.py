...
def test_app_version_matches_latest_changelog() -> None:
    assert APP_VERSION == latest_changelog_entry()["version"] == "1.3.0"
...
def test_unpublished_public_changelog_entries_fill_version_gaps_in_order() -> None:
    ...
    assert [entry["version"] for entry in entries] == ["1.1.2", "1.1.4", "1.1.6", "1.1.7", "1.1.8", "1.1.9", "1.1.10", "1.1.11", "1.1.12", "1.1.13", "1.1.14", "1.1.15", "1.1.16", "1.2.0", "1.2.1", "1.2.2", "1.2.3", "1.2.4", "1.3.0"]


def test_first_publication_only_sends_latest_changelog() -> None:
    entries = unpublished_public_changelog_entries(set())
    assert [entry["version"] for entry in entries] == ["1.3.0"]
